...
CE.ratio <- function(all.web.info){
    ...
    r.b.mid = 1
    r.b.factor <- 2
    up.rb <- r.b.factor^seq(-8, 2, by=4)
    rbs <- r.b.mid*up.rb
    
    ratio.results = matrix(0, length(rbs)*length(ais)*length(ajs), 8)
    row.counter = 1
    ...
    for(r.b in rbs)
        for(ai in ais)
            for(aj in ajs){
                ...
                parms <- list(e, n, ni, r.a, M, S, target.C, real.web) 
                opt <- c(ai, aj, r.b)
                
                temp <- ratio.power(opt, parms, optimizer=F)
                ratio.yy <- temp[1]
                a = temp[2]
                if(ratio.yy>-0.5){
                    ## check to make sure target.C is possible given r.b
                ...
                if(ratio.yy>best){
                    best = ratio.yy
                    ratio.results = c(target.C, pred.C, r.a, r.b, ai, aj, ratio.yy, a)
                    best.web=web
                    best.EHL=EHL
                }
            }    
    list(power=best, parms=list(e=e, n=n, ni=ni, a=a, ai=ai, aj=aj, r.a=r.a, r.b=r.b), EHL=best.EHL, web=best.web)
}



NM.ratio <- function(all.web.info, init.par.file="ratio.initial.pars.txt"){

    ratio.initial.pars <- read.csv(init.par.file, header=T)
    ##ratio.initial.pars <- read.csv("~/allometric.web/data/NM.optim/ratio.initial.pars.txt", header=T)
...
Randomise.ADBM <- function(x, all.web.info){

    if(!is.list(x[[1]]))
        result <- inner.Randomise.ADBM(x, all.web.info)

    if(is.list(x[[1]])){
        result <- list(1)
        for(i in 1:length(x))
            result[[i]] <- inner.Randomise.ADBM(x[[i]], all.web.info)
    }
    result
}

inner.Randomise.ADBM <- function(x, all.web.info){

    EHL <- x$EHL
    EHL[[1]] <- EHL[[1]][order(runif(length(EHL[[1]])))]
    EHL[[2]] <- matrix(EHL[[2]][order(runif(length(EHL[[2]])))], length(EHL[[1]]), length(EHL[[1]]))
    EHL[[3]] <- matrix(EHL[[3]][order(runif(length(EHL[[3]])))], length(EHL[[1]]), length(EHL[[1]]))
    web <- Get.web(EHL)
    power <- Compare.links(all.web.info$predation.matrix, web)
    list(power=power, pars=NA, EHL=EHL, web=web)
